...
{
    public static class H2mccCompressionV13
    {
        ...
        public static bool Decompress(Stream compressed, Stream decompressTo)
        {
            ...
            var header = BlamSerializer.Deserialize<H2mccV13MapHeader>(compressed);

            var sections = new H2mccCompressionV13Sections.CompressionSection[header.CompressionIndexCount];
            for (var i = 0; i < sections.Length; i++)
            {
                var offset = header.CompressionIndexOffset + BlamSerializer.SizeOf<H2mccCompressionV13Sections.CompressionSection>() * i;
                var sect = H2mccCompressionV13Sections.CompressionSection.ReadCompressionSection(compressed, offset);
                sections[i] = sect;
            }

            var headerb = new Span<byte>(new byte[896]);

            compressed.Position = 0;
            compressed.Read(headerb);
            decompressTo.WriteUInt32(DecompressedFourCC);
            decompressTo.Write(headerb.Slice(4));
