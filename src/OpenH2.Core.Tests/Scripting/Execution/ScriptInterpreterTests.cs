...
namespace OpenH2.Core.Tests.Scripting.Execution
{
    public class ScriptInterpreterTests
    {
        ...
        {
            var stringData = new byte[13];
            // { h i 0 h e y 0 h e l l o 0 }

            Array.Copy(Encoding.UTF8.GetBytes("hi"), 0, stringData, 0, 2);
            Array.Copy(Encoding.UTF8.GetBytes("hey"), 0, stringData, 3, 3);
            Array.Copy(Encoding.UTF8.GetBytes("hello"), 0, stringData, 7, 5);

            var scen = new ScenarioTag(0)
            {
                ScriptStrings = stringData,
                ScriptSyntaxNodes = new ScenarioTag.ScriptSyntaxNode[]
                {
