...
namespace OpenH2.Serialization
{
    public class BlamSerializer
    {
        ...
        /// </summary>
        public BlamSerializer(Memory<byte> data)
        {
            this.data = data;
        }
        ...
        {
            return Deserialize<T>(this.data.Span, instanceStart, staticOffset);
        }

        public static object Deserialize(Type type, Span<byte> allData, int instanceStart = 0, int staticOffset = 0)
        {
            if (deserializers.TryGetValue(type, out var deser))
            {
                return deser(allData, instanceStart, staticOffset);
            }

            throw new NotSupportedException($"Type '{type.Name}' was not found in the deserializer collection");
        }

        public static T Deserialize<T>(Span<byte> allData, int instanceStart = 0, int staticOffset = 0)
        {
            return (T)Deserialize(typeof(T), allData, instanceStart, staticOffset);
        }
    }
}
