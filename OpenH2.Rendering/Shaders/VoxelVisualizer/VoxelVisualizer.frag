...
layout (binding = 0) uniform sampler3D texture3D; // Texture in which voxelization is stored.
layout (binding = 1) uniform sampler2D textureBack; // Unit cube back FBO.
layout (binding = 2) uniform sampler2D textureFront; // Unit cube front FBO.
...
void main() {
	...
	// Initialize ray.
	const vec3 origin = isInsideCube(Globals.ViewPosition, 0.2f) ? 
		Globals.ViewPosition : texture(textureFront, textureCoordinateFrag).xyz;
